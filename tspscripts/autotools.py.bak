...
class DsfJob(object):
    ...
    def _list_of_strings_to_list_of_lists(self, list_of_strings):
        ...
        for i in range(0, len(list_of_lists)):
            ...
            list_of_lists[i]=map(float, list_of_lists[i])
        #
    ...
    def _read_instructions(self, input_file):
        ...
        plng_list=map(int, instructions[:,4].tolist()) # list of integers
        qlis_list=self._list_of_strings_to_list_of_lists(instructions[:,5]) 
        #                                                 list of lists
        dq_list=map(float, instructions[:,6].tolist()) # list of floats
        maxm_list=map(float, instructions[:,7].tolist()) # list of.. whatever
   ...
   # to be of the same family. It could be costly, however, to do the out_data for every single one.
    ...
    def find_nframes(self, trj_idxs):
        ...
        try:
            ...
            raise TypeError
        #
        dictIdx2Nframes=dict(zip(trj_idxs, trj_nframes))
        #
        for i in trj_idxs:
            if i < self.get_nfiles():                
...
#        found on basis of their sampling characteristics        
        ...
        try:
            ...
            raise TypeError
        #
        sampling_info=zip(trj_ts, trj_base, trj_samples_bef_incr, trj_samples_bef_linstep, trj_linstep)
        dictIdx2SamplingInfo=dict(zip(trj_idxs, sampling_info))
        #
