...
Ttoclr=lambda T,minT,maxT,cmap=cmapc2h: cmap(int((T-minT)*1.0/(maxT-minT)*255))
...
def find_idcs(series, lims):
    ...
    as present.
    """
    tolr=abs(series[1,0]-series[0,0])
    lims=sorted(lims)
...
def simple_baseline(series, xlim):
    ...
    """
    tolr=abs(series[1,0]-series[0,0]) # tolerance for indexing = separation between entries
...
def plot_series_from_dict_ocd(datasdict, labeldict, colordict=None, markerdict=None, order_to_key=None, weed=1,**kwargs):
        ...
        """
        if colordict==None: colordict={key:clrs.next()}
        if markerdict==None: markerdict={key:mrks.next()}
...
class CorAnalysis(object):
	...
	def check_cor_integral(self, abs_corr_threshold=1.1, frac_int_threshold=0.5):
		corfs=self.get_corfs()
		check=self.get_check()
		corints={c:np.trapz(corfs[c][:,1], corfs[c][:,0]) for c in corfs}
		self.corints=corints
		average_int=np.average([corints[c] for c in corints])
