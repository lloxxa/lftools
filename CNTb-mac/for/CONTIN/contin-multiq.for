...
C----------------------------------------------------------------------------
C
C  LUSER(29) = T for 2 grids as follows:
C
C  Grid 1 is with the decay rates independent of the data set (e.g., 
C         independent of angle); i.e., the exponential in the kernel 
C         is of the form 
C                        exp[-(time)*(decay rate)]
C
C  Grid 2 is with the decay rates dependent on the data set as follows:
C         exp[-(time)*(decay rate)*RUSER(50+IDSET)]
C         RUSER(51)=1. is imposed by CONTIN;
C         You must input RUSER(52),...,RUSER(50+IUSER(20)).
C         Thus, for the usual case of a translational-diffusional grid, and 
C           q(IDSET) the magnitude of the scattering vector, for data set 
C           IDSET,
C           RUSER(50+IDSET) = [q(IDSET)/q(1)]**2, 
C           and a grid point on Grid 2 with a value G has a diffusion 
C           coefficient D = G/q(1)**2.
C         You can choose any of your data sets to be data set 1; choosing one 
C           with a q around sqrt(q_max*q_min) often efficiently utilizes the 
C           range of the grid.
C
C  For a double grid, you must input the following:
C    LUSER(29) = T
C    IQUAD = 1
C    IUSER(20) > 1 the no. of data sets (as in the Splice package)
C    DOUSNQ=T, NONNEG=T (as always with the Splice Package)  
C    NG must be even; it is the total number of grid points; i.e., twice the 
C       number in each grid.
C    RUSER(52),...,RUSER(50+IDSET) (as specified above)
C    IUSER(10) is ignored; you always have a kernel of the form:
C              FORMF2*exp[-(time)*(decay rate)].  You can modify this in 
C              USERK.
C--------------------------------------------------------------------------
C  
C  Test run:
C  =========
C    The file contin-multiq.in contains the test data for the Splice Package, 
C      but with an added component with amplitudes of 0.4 and 0.2 and decay 
C      rates of 0.02 and 0.03, respectively, in the 1st and 2nd data sets.  
C      Thus RUSER(52)=1.5 is appropriate, and this added component should 
C      appear on the 2nd (diffusional) grid with an amplitude of 0.2 and a 
C      decay rate of 0.02.  The 2 remaining components in the original Spline 
C      test data should appear as in Update 2 in the 1st 
C      (data-set-independent) grid.
C
C  Obviously, you have the best chance of resolving q-independent from 
C    diffusional components if your RUSER(52),...,RUSER(50+IUSER(20)) covers 
C    a wide range.
C--------------------------------------------------------------------------
C  
C  New diagnostics: 
C  ================
C    FORMF2 1: Same as MULTIW 7 (in the Splice package).
C    SETGRD 4: When LUSER(29)=T, you must have IQUAD=1, IUSER(20), and 
C              NG even (as specified above).
C    USERK  9: Same as MULTIW 7 (in the Splice package).
C    If an old diagnostic tells you to increase MDSET or MG1 in a PARAMETER 
C      statement, then you must also increase this in the new PARAMETER 
C      statement in subprogram FORMF2.
C--------------------------------------------------------------------------
C
C  Some corrections to the Splice Package documentation (CONTIN Update 2):
C  (1) The Splice Package will determine the scale factors to consistently 
C        scale your data sets together, but you should prescale your input 
C        data so that these scale factors already agree to within about a 
C        factor of 1.5 (better 1.2); otherwise the weighting is too strongly 
C        distorted.  You should be able to achieve this prescaling by eye or 
C        with a simple automatic procedure.
C  (2) The "Incompatability Ratio" discussed in Sec. G is not useful, since it 
C      will always get worse (greater than 1.0) as the noise level decreases.  
C      A better criterion is simply the SD(global) values; they tell how 
C      well the global model can fit each data set.
C--------------------------------------------------------------------------
C  Changes to the Splice Package are in lower case.
C  Changes for the double grid have a "!g2" at the end of the line.
C  Changes for the form factors have a "!ff" at the end of the line.
C--------------------------------------------------------------------------
C
...
C      WRITYT, USERSX, USERNQ, SETNNG, ANALYZ, SETWT, GETYLY                    
...
C      BETAIN, PLPRIN, USEROU, MOMENT, MOMOUT, RUNRES, GETPRU,                  
...
C***********************************************************************    0047
      PARAMETER (MDSET=30)                                                      
      EXTERNAL BETAIN, DIFF, FFLAT, FISHNI, FORMF2, GAMLN, PGAUSS, 
     1         RANDOM, USEREX, USERK, USERLF, USERTR
      DIMENSION STDVWT(MDSET)                                                   
...
C                                                                           0055
      DIMENSION T(5000), SQRTW(5000), Y(5000), EXACT(5000), YLYFIT(5000)!g2
      DIMENSION G(300), CQUAD(300), VK1Y1(300), S(300,3), VALPHA(300),!g2
     1 VALPCV(300), SOLUTN(300), IISIGN(300), SOLBES(300),!g2
     2 AA(300,300), SSCALE(300)!g2
      DIMENSION AINEQ(300,300), RHSNEQ(300), LBIND(300)!g2
      DIMENSION A(300,300), IWORK(300)!g2
      DIMENSION REG(300,300)!g2
      DIMENSION AEQ(100,300), PIVOT(100)!g2
      DIMENSION WORK(90898)!g2
...
C                                                                           0080
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     0083
     2 SRANGE                                                               0084
...
C                                                                           0099
      DATA MY/5000/, MA/300/, MG/300/, MREG/300/, MINEQ/300/, MEQ/100/,!g2
     1 MDONE/90/, MWORK/90898/!g2
C                                                                           0102
...
C-----------------------------------------------------------------------    0129
      CALL SETGRD (CQUAD,G,GMNMX,IGRID,IQUAD,MG,NG,NOUT,luser(29),!g2
     1             iuser(20))!g2
C-----------------------------------------------------------------------    0131
...
C                                                                               
      IF (IUSER(19).GE.1 .OR. IUSER(20).GE.2 .OR.                               
     1    (IUSER(20).EQ.1 .AND. IWT.NE.1 .AND. IWT.NE.4)) THEN                  
C        ---------------------------------------------------------------        
C        Use Splice Package.                                                    
C        Compute the weights for the IUSER(20) data sets and balance            
C        them.  Use adaptive weighting if IUSER(19)>0.                          
C        ---------------------------------------------------------------        
         DO 150 JGL=1,NGL                                                       
            SSCALE(JGL)=1.D0                                                    
  150    CONTINUE                                                               
         IF (IUSER(20).GE.2 .AND. .NOT.LUSER(10)) THEN                          
C           ------------------------------------------------------------        
C           A second analysis with rescaled weights will be made.               
C           Temporarily reduce output for the first analysis.                   
C           ------------------------------------------------------------        
            PRWTSV=PRWT                                                         
            PRWT=.FALSE.                                                        
            IPRISV=IPRINT(2)                                                    
            ...
            IPLFIT(2)=0                                                         
         END IF                                                                 
         CALL MULTIW (CQUAD,G,IWORK,MA,MDSET,MG,MWORK,                          
     1                   SOLUTN,SQRTW,STDVWT,T,WORK,Y,YLYFIT)                   
         CALL ANALYZ (2,                                                        
     ...
     4   Y,YLYFIT)                                                              
         IF (IUSER(20).GE.2 .AND. .NOT.LUSER(10)) THEN                          
C           ------------------------------------------------------------        
C           Rescale SQRTW, and restore output Control Variables for             
C           final analysis.                                                     
C           ------------------------------------------------------------        
               PRWT=PRWTSV                                                      
            IF (PRWT) WRITE (NOUT,5310)                                         
 5310       FORMAT (//30X,'Square roots of refined least-squares ',             
     1                    'weights')                                            
            JYSTRT=1                                                            
            DO 310 JDSET=1,IUSER(20)-1                                          
               JYEND=IUSER(20+JDSET)                                            
               LG=NGL-IUSER(20)+1+JDSET                                         
               FSCALE=1./(1.+SOLBES(LG)*SSCALE(LG))                             
               DO 320 JY=JYSTRT,JYEND                                           
                  SQRTW(JY)=SQRTW(JY)*FSCALE                                    
  320          CONTINUE                                                         
               IF (PRWT) WRITE (NOUT,5320) (SQRTW(JY),JY=JYSTRT,                
     1                                                   JYEND)                 
 5320          FORMAT (/(1X,1P10E13.4))                                         
               JYSTRT=JYEND+1                                                   
  310       CONTINUE                                                            
            IF (PRWT) WRITE (NOUT,5320) (SQRTW(JY),JY=JYSTRT,NY)                
            IPRINT(2)=IPRISV                                                    
            IPLRES(2)=IRESSV                                                    
            IPLFIT(2)=IFITSV                                                    
            CALL ANALYZ (2,                                                     
     ...
     4      Y,YLYFIT)                                                           
         END IF                                                                 
C        ---------------------------------------------------------------        
C        Output summary of global splicing.                                     
C        ---------------------------------------------------------------        
         CALL GETYLY (SOLBES,                                                   
     1                CQUAD,G,IUNIT,IWT,MWORK,NG,NGL,NLINF,NY,SQRTW,            
     2                SSCALE,T,WORK,Y,YLYFIT)                                   
 5350    FORMAT (///' Data set',3X,'SD(global)',3X,'SD(single)',                
     1              3X,'Incompatibility Ratio')                                 
         WRITE (NOUT,5350)                                                      
         JYSTRT=1                                                               
         DO 350 JDSET=1,MAX0(1,IUSER(20))                                       
            JYEND=IUSER(20+JDSET)                                               
            STDDEV=0.                                                           
            DO 360 JY=JYSTRT,JYEND                                              
               STDDEV=STDDEV+YLYFIT(JY)**2                                      
  360       CONTINUE                                                            
            STDDEV=SQRT(STDDEV/FLOAT(JYEND-JYSTRT+1))                           
            SDABS=STDDEV*STDVWT(JDSET)                                          
 5360       FORMAT (1X,I8,1P2E13.3,E12.2)                                       
            WRITE (NOUT,5360) JDSET,SDABS,STDVWT(JDSET),STDDEV                  
            JYSTRT=JYEND+1                                                      
  350    CONTINUE                                                               
      ELSE                                                                      
C                                                                               
C        ---------------------------------------------------------------        
C        A standard CONTIN analysis, without calling MULTIW, will be            
C        done.                                                                  
C        ---------------------------------------------------------------        
...
C                                                                               
      ...
      STOP                                                                  0181
      END                                                                   0182
C++++++++++++++++ Multiq VERSION 2DP (March 1996) ++++++++++++++     
C  SUBROUTINE MULTIW.  Computes the weights for the NDSET data sets and         
C    balances them from one data set to the next so that the                    
C    squared residuals (weighted deviations between the fit and the             
C    data) averaged over each data set are equal, and scales them so            
C    that STD. DEV. in the final weighted analysis would be 1.0 if there        
C    were no inconsistencies between the data sets.                             
C                                                                               
C                                                                               
C  MULTIW cannot be used in all cases that CONTIN can.  Your problem            
C    must satisfy the following restrictions:                                   
C                                                                               
C       NLINF (see Sec. 4.1.2.4 of the Users Manual) can only have the          
C             following 3 values:                                               
C       NLINF = 2*NDSET-1 for 1 different parameter (typically a                
C                         constant baseline or "dust term") for each            
C                         of the NDSET data sets.                               
C                         This is probably the most common case.                
C                         (The last NDSET-1 parameters are always used          
C                          to rescale the first NDSET-1 data sets for           
C                          optimal splicing.)                                   
C             = NDSET for the same (e.g., baseline) parameter for               
C                     all the data sets.  (Only reasonable if the               
C                     baseline term is the same and reproducible                
C                     over all data sets.)  The baseline parameter is           
C                     assumed to be the same AFTER proper rescaling;            
C                     i.e., it has the same magnitude relative to the           
C                     rest of the distribution in each of the data sets.        
C             = NDSET-1 for no baseline parameter at all.                       
C                                                                               
C       In addition, you must input                                             
C         NONNEG=T and DOUSNQ=T; i.e., only nonnegativity constraints on        
C                                all parameters, except the last NDSET-1        
C                                (scale) parameters.                            
C         NEQ = 0 (no equality constraints).                                    
C                                                                               
C  Input Control Variables:                                                     
C  =======================                                                      
C  IUSER(19) > 0 to do adaptive weighting, with the weight for a given          
C                point computed from the scatter in the fit of the              
C                Reference Solution to the data in a window                     
C                2*IUSER(19)+1 data points wide centered at the given           
C                data point.                                                    
C                                                                               
C  IUSER(20) = NDSET, the number of data sets.  The data sets are simply        
C                     input concatenated into a single Y array and a            
C                     single T array.                                           
C            = 1 means there is only one data set, but MULTIW will still        
C                be called, to use the faster computation of weights.           
C            < 1 will not call MULTIW, unless adaptive weights are to be        
C                computed.                                                      
C            > MDSET causes an error message and abort.  MDSET is set in        
C                    PARAMETER statement below.  With the current               
C                    dimensions of IUSER, MDSET cannot exceed 30.               
C                                                                               
C  IUSER(J), J=21,22,...,20+IUSER(20), = the subscript of the last data         
C                                        point of data set J in the Y           
C                                        and T arrays (i.e., the                
C                                        boundary or maximum subscript).        
C  IUSER(21) .LE. 0 (instead of the boundary subscript for the first            
C                   data set) will cause IUSER(J),                              
C                   J=21,22,...,20+IUSER(20) to be set automatically by         
C                   searching for monotonicity violations in T.  This           
C                   option can only be used if T in each data set is            
C                   monotonically increasing and if each successive pair        
C                   of data sets overlaps in T.  Otherwise, you will            
C                   have to input these IUSER(21),... yourself.                 
C                                                                               
C                                                                               
C  PARAMETER constants                                                          
C  ===================                                                          
C  If your problem is too large, you may have to adjust the PARAMETER           
C    constants in the PARAMETER statement below as follows:                     
C                                                                               
C  MDSET = the maximum number of data sets.  If you want to set                 
C          MDSET>30, then you must increase the dimension of IUSER and          
C          make the changes described in Sec. 4.1.1.                            
C  MY1 = the maximum number of data points in a single data set.                
C  MG1 .GE. NG+1 (NG = the no. of grid points).                                 
C  ---------------------------------------------------------------------        
C  Calls subprograms - ERRMES, FIT1Y, USERWT                                    
C  which in turn call - USERK, USERTR, USERLF, NNLS, DIFF, H12, G1, G2          
C  ---------------------------------------------------------------------        
      SUBROUTINE MULTIW (CQUAD,G,IWORK,MA,MDSET,MG,MWORK,                       
     1                   SOLUTN,SQRTW,STDVWT,T,WORK,Y,YLYFIT)                   
C     ******************************************************************        
      PARAMETER (MY1=200, MG1=44)                                               
C     ******************************************************************        
      DOUBLE PRECISION PRECIS, RANGE                                            
      DOUBLE PRECISION ACO, RHS1, SOLUTN, WORK, WRKNY1                          
      LOGICAL DOCHOS, DOMOM, DOUSIN, DOUSNQ, LAST, NEWPG1,                      
     1 NONNEG, ONLY1, PRWT, PRY, SIMULA, LUSER                                  
      LOGICAL MULTIL                                                            
      EXTERNAL BETAIN, DIFF, FFLAT, FISHNI, FORMF2, GAMLN, PGAUSS, 
     1         RANDOM, USEREX, USERK, USERLF, USERTR
      DIMENSION CQUAD(MG), G(MG), IWORK(MA), SOLUTN(MG), SQRTW(*),              
     1          STDVWT(MDSET), T(*), WORK(MWORK), Y(*), YLYFIT(*)               
      DIMENSION ACO(MY1,MG1), IHOLER(6), RHS1(MY1), RWORK(MY1),                 
     1          WRKNY1(MY1)                                                     
      EQUIVALENCE (WRKNY1,RWORK)                                                
     ...
     2 LUSER(30)                                                                
      DATA IHOLER/1HM, 1HU, 1HL, 1HT, 1HI, 1HW/                                 
      NDSET=MAX0(1,IUSER(20))                                                   
      IF (NDSET .GT. MDSET) CALL ERRMES (1,.TRUE.,IHOLER,NOUT)                  
      IF (IUSER(21).LE.0 .AND. NDSET.GT.1) THEN                                 
C        ---------------------------------------------------------------        
C        Compute boundaries of the data sets from monotonicity                  
C        violations of T.  This will only work if successive data sets          
C        overlap in T, and if T in each data set is monotonically               
C        increasing.  Else, an error message and an abort occurs, and           
C        you must input the boundaries as IUSER(J), J=21,...                    
C        ---------------------------------------------------------------        
         JDSET=1                                                                
         DO 110 JY=1,NY-1                                                       
            IF (T(JY) .GT. T(JY+1)) THEN                                        
               IUSER(20+JDSET)=JY                                               
               JDSET=JDSET+1                                                    
               IF (JDSET .GT. MDSET) CALL ERRMES (2,.TRUE.,IHOLER,NOUT)         
            END IF                                                              
  110    CONTINUE                                                               
         IF (JDSET .NE. NDSET) CALL ERRMES (3,.TRUE.,IHOLER,NOUT)               
      END IF                                                                    
      IUSER(20+NDSET)=NY                                                        
C     ------------------------------------------------------------------        
C     Check that restrictions are satisfied.                                    
C     ------------------------------------------------------------------        
      IF (.NOT.NONNEG .OR. NEQ.GT.0) CALL ERRMES (4,.TRUE.,IHOLER,              
     1                                            NOUT)
      MULTIL=NLINF .EQ. 2*NDSET-1                                               
      if (luser(29)) then!g2
         ng1grd=ng/2!g2
      else!g2
         ng1grd=ng!g2
      end if!g2
      IF (NLINF .EQ. NDSET-1) THEN                                              
         NGL1=NG1grd!g2
         LLINF=0                                                                
      ELSE IF (MULTIL) THEN                                                     
         NGL1=NG1grd+1!g2
      ELSE IF (NLINF. EQ. NDSET) THEN                                           
         NGL1=NG1grd+1!g2
         LLINF=1                                                                
      ELSE                                                                      
         CALL ERRMES (5,.TRUE.,IHOLER,NOUT)                                     
      END IF                                                                    
      IF (NGL1.EQ.NG1grd+1 .AND. .NOT.DOUSNQ) CALL ERRMES (6,.TRUE.,!g2
     1                                                     IHOLER,NOUT)!g2
      JYSTRT=1                                                                  
      DO 210 JDSET=1,NDSET                                                      
         JYEND=IUSER(20+JDSET)                                                  
         NY1=JYEND-JYSTRT+1                                                     
         IF (NY1.LE.0 .OR. NY1.GT.MIN0(MY1,NY) .OR. NGL1.GT.MG1) CALL           
     1        ERRMES (7,.TRUE.,IHOLER,NOUT)                                     
         IF (MULTIL) LLINF=JDSET                                                
         IF (IWT.NE.1 .AND. IWT.NE.4) THEN                                      
C           ------------------------------------------------------------        
C           Compute initial estimates of weights.                               
C           ------------------------------------------------------------        
            CALL FIT1Y (JYSTRT,JYEND,LLINF,NDF,NY1,NGL1,ng1grd,!g2
     1                  ACO,CQUAD,G,IWORK,MY1,RHS1,SOLUTN,                      
     2                  SQRTW,STDVWT(JDSET),T,WORK,WRKNY1,Y,YLYFIT)             
C           ------------------------------------------------------------        
C           Do the same as in SETWT, except for restricted range in JY.         
C           ------------------------------------------------------------        
            ERRFIT=0.                                                           
            IF (NERFIT .GT. 0) THEN                                             
               ABSMIN=SRANGE                                                    
               DO 220 JY=JYSTRT,JYEND                                           
                  DUM=ABS(Y(JY)-YLYFIT(JY))                                     
                  IF (DUM .LT. ABSMIN) THEN                                     
                     ABSMIN=DUM                                                 
                     L=JY                                                       
                  END IF                                                        
  220          CONTINUE                                                         
               KYMAX=MIN0(JYEND,L+NERFIT/2)                                     
               KYMIN=MAX0(JYSTRT,KYMAX-NERFIT+1)                                
               DO 230 KY=KYMIN,KYMAX                                            
                  ERRFIT=ERRFIT+YLYFIT(KY)**2                                   
  230          CONTINUE                                                         
               ERRFIT=SQRT(ERRFIT/FLOAT(KYMAX-KYMIN+1))                         
            END IF                                                              
            IF (IWT .EQ. 5) THEN                                                
C              ---------------------------------------------------------        
C              Save NY and call USERWT with NY=NY1 in COMMON and                
C              with offset arrays.                                              
C              ---------------------------------------------------------        
               NYSAVE=NY                                                        
               NY=NY1                                                           
               CALL USERWT (Y(JYSTRT),YLYFIT(JYSTRT),NY1,ERRFIT,                
     1                      SQRTW(JYSTRT))                                      
               NY=NYSAVE                                                        
            ELSE IF (IWT.EQ.2 .OR. IWT.EQ.3) THEN                               
               DO 240 JY=JYSTRT,JYEND                                           
                  YFIT=AMAX1(ABS(Y(JY)-YLYFIT(JY)),ERRFIT)                      
                  IF (YFIT .LE. 0.) CALL ERRMES (8,.TRUE.,IHOLER,NOUT)          
                  SQRTW(JY)=1./YFIT                                             
                  IF (IWT .EQ. 2) SQRTW(JY)=SQRT(SQRTW(JY))                     
  240          CONTINUE                                                         
            END IF                                                              
         END IF                                                                 
C        ---------------------------------------------------------------        
C        Compute fit to data with unrefined weights.                            
C        ---------------------------------------------------------------        
         CALL FIT1Y (JYSTRT,JYEND,LLINF,NDF,NY1,NGL1,ng1grd,!g2
     1               ACO,CQUAD,G,IWORK,MY1,RHS1,SOLUTN,                         
     2               SQRTW,STDVWT(JDSET),T,WORK,WRKNY1,Y,YLYFIT)                
         IF (IUSER(19) .GE. 1) THEN                                             
C           ------------------------------------------------------------        
C           Compute adaptive weights.                                           
C           ------------------------------------------------------------        
            IF (IUSER(19).GE.NY1/2 .OR. IUSER(19).LE.1) CALL ERRMES (9,         
     1                                             .FALSE.,IHOLER,NOUT)         
C           ------------------------------------------------------------        
C           Save SQRTW in RWORK.                                                
C           ------------------------------------------------------------        
            K=0                                                                 
            DO 250 JY=JYSTRT,JYEND                                              
               K=K+1                                                            
               RWORK(K)=SQRTW(JY)                                               
  250       CONTINUE                                                            
            JWORK=0                                                             
            DO 260 JY=JYSTRT,JYEND                                              
               KSTART=MAX0(JYSTRT,JY-IUSER(19))                                 
               KEND=MIN0(JYEND,JY+IUSER(19))                                    
               KWORK=KSTART-JYSTRT                                              
               VARJ=0.                                                          
               DO 270 K=KSTART,KEND                                             
                  KWORK=KWORK+1                                                 
                  VARJ=VARJ+(RWORK(KWORK)*YLYFIT(K))**2                         
  270          CONTINUE                                                         
               SIGAVG=SQRT(VARJ/(KEND-KSTART+1))                                
               IF (SIGAVG .LE. 0.) CALL ERRMES (10,.TRUE.,IHOLER,NOUT)          
               JWORK=JWORK+1                                                    
               SQRTW(JY)=RWORK(JWORK)/SIGAVG                                    
  260       CONTINUE                                                            
         END IF                                                                 
C        ---------------------------------------------------------------        
C        Scale weights so that STD. DEV. for each data set is 1.0.              
C        ---------------------------------------------------------------        
         VARJ=0.                                                                
         DO 280 JY=JYSTRT,JYEND                                                 
            VARJ=VARJ+(SQRTW(JY)*YLYFIT(JY))**2                                 
  280    CONTINUE                                                               
         IF (NY1.LE.NDF .OR. VARJ.LE.0.) CALL ERRMES (11,.TRUE.,IHOLER,         
     1                                                NOUT)                     
         FACTOR=SQRT(FLOAT(NY1-NDF)/VARJ)                                       
         DO 290 JY=JYSTRT,JYEND                                                 
            SQRTW(JY)=SQRTW(JY)*FACTOR                                          
  290    CONTINUE                                                               
         NDFTOT=NDFTOT+NDF                                                      
         JYSTRT=JYEND+1                                                         
  210 CONTINUE                                                                  
      IF (PRWT) THEN                                                            
         WRITE (NOUT,5410)                                                      
 5410    FORMAT (//30X,'Square roots of normalized least-squares',              
     1           ' weights')                                                    
         JYSTRT=1                                                               
         DO 410 JDSET=1,NDSET                                                   
            JYEND=IUSER(20+JDSET)                                               
            WRITE (NOUT,5420) (SQRTW(JY),JY=JYSTRT,JYEND)                       
 5420       FORMAT (/(1X,1P10E13.4))                                            
            JYSTRT=JYEND+1                                                      
  410    CONTINUE                                                               
      END IF                                                                    
      RETURN                                                                    
      END                                                                       
C++++++++++++++++ Multiq VERSION 2DP (March 1996) ++++++++++++++     
C  SUBROUTINE FIT1Y.  Computes the Reference Solution, using NNLS only;         
C    i.e., assuming no equality constraints and only nonnegativity              
C    constraints on all NGL1 parameters.                                        
C                                                                               
C  ---------------------------------------------------------------------        
C  Calls subprograms - ERRMES, USERK, USERLF, NNLS                              
C  which in turn call - USERTR, DIFF, H12, G1, G2                               
C  ---------------------------------------------------------------------        
      SUBROUTINE FIT1Y (JYSTRT,JYEND,LLINF,NDF,NY1,NGL1,ng1grd,!g2
     1                  ACO,CQUAD,G,IWORK,MY1,RHS1,SOLUTN,                      
     2                  SQRTW,STDDEV,T,WORK,WRKNY1,Y,YLYFIT)                    
      DOUBLE PRECISION PRECIS, RANGE                                            
      DOUBLE PRECISION ACO, DUB, RHS1, SOLUTN, SWT, WORK, WRKNY1,               
     1                 ZERO                                                     
      LOGICAL DOCHOS, DOMOM, DOUSIN, DOUSNQ, LAST, NEWPG1,                      
     1 NONNEG, ONLY1, PRWT, PRY, SIMULA, LUSER                                  
      EXTERNAL BETAIN, DIFF, FFLAT, FISHNI, FORMF2, GAMLN, PGAUSS, 
     1         RANDOM, USEREX, USERK, USERLF, USERTR
      DIMENSION ACO(MY1,NGL1), CQUAD(NG), G(NG), IWORK(NGL1),                   
     1          RHS1(NY1), SOLUTN(NGL), SQRTW(*), T(*),                         
     2          WORK(2*NG+2), WRKNY1(NY1), Y(*), YLYFIT(*)                      
      DIMENSION IHOLER(6)                                                       
      COMMON /DBLOCK/ PRECIS, RANGE                                             
      COMMON /SBLOCK/ DFMIN, SRMIN,                                             
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),         
     ...
     1 NEWPG1, NONNEG, ONLY1, PRWT, PRY, SIMULA,                                
     2 LUSER(30)                                                                
      DATA IHOLER/1HF, 1HI, 1HT, 1H1, 1HY, 1H /                                 
C     DRABS(ZERO)=ABS(ZERO)                                                     
      DRABS(ZERO)=DABS(ZERO)                                                    
      ZERO=0.D0                                                                 
C     ------------------------------------------------------------------        
C     WORK(ICOL), ICOL=1,NGL1 will be used internally as crude scale            
C                             factors.                                          
C     ------------------------------------------------------------------        
      DO 100 ICOL=1,NGL1                                                        
         WORK(ICOL)=0.D0                                                        
  100 CONTINUE                                                                  
      K=0                                                                       
      DO 110 IROW=JYSTRT,JYEND                                                  
         K=K+1                                                                  
         SWT=SQRTW(IROW)                                                        
         IIROW=IROW                                                             
         DO 120 ICOL=1,NG1grd!g2
            IICOL=ICOL                                                          
            ACO(K,ICOL)=CQUAD(ICOL)*SWT*USERK(IIROW,T,IICOL,G)                  
            WORK(ICOL)=WORK(ICOL)+DRABS(ACO(K,ICOL))                            
  120    CONTINUE                                                               
         IF (LLINF .GT. 0) THEN                                                 
            ACO(K,NGL1)=SWT*USERLF(IIROW,LLINF,T,NY)                            
            WORK(NGL1)=WORK(NGL1)+DRABS(ACO(K,NGL1))                            
         END IF                                                                 
         RHS1(K)=SWT*Y(IROW)                                                    
  110 CONTINUE                                                                  
      DO 130 ICOL=1,NGL1                                                        
         IF (WORK(ICOL) .LE. ZERO) GO TO 130                                    
         WORK(ICOL)=1.D0/WORK(ICOL)                                             
         K=0                                                                    
         DO 140 JY=JYSTRT,JYEND                                                 
            K=K+1                                                               
            ACO(K,ICOL)=ACO(K,ICOL)*WORK(ICOL)                                  
  140    CONTINUE                                                               
  130 CONTINUE                                                                  
      CALL NNLS (ACO,MY1,NY1,NGL1,RHS1,SOLUTN,DUB,WORK(NG+2),WRKNY1,            
     1           IWORK,IERROR,RANGE)                                            
      IF (IERROR .NE. 1) CALL ERRMES (1,.TRUE.,IHOLER,NOUT)                     
C     ------------------------------------------------------------------        
C     Count NDF, the number of degrees of freedom.                              
C     ------------------------------------------------------------------        
      NDF=0                                                                     
      DO 210 JG=1,NGL1                                                          
         IF (SOLUTN(JG) .GT. ZERO) NDF=NDF+1                                    
  210 CONTINUE                                                                  
      IF (NDF .GE. NY1) THEN                                                    
      ...
      ELSE                                                                      
         STDDEV=DUB                                                             
         STDDEV=STDDEV/SQRT(FLOAT(NY1-NDF))                                     
      END IF                                                                    
C     ------------------------------------------------------------------        
C     Put the unweighted residuals in YLYFIT.                                   
C     ------------------------------------------------------------------        
      DO 220 JY=JYSTRT,JYEND                                                    
         JJY=JY                                                                 
         YFIT=0.                                                                
         DO 230 JG=1,NG1grd!g2
            JJG=JG                                                              
            YFIT=YFIT+SOLUTN(JG)*WORK(JG)*CQUAD(JG)*USERK(JJY,T,JJG,G)          
  230    CONTINUE                                                               
         IF (LLINF .GT. 0) YFIT=YFIT+SOLUTN(NGl1)*WORK(NGl1)*!d2
     1                               USERLF(JJY,LLINF,T,NY)                     
         YLYFIT(JY)=Y(JY)-YFIT                                                  
  220 CONTINUE                                                                  
      RETURN                                                                    
      END                                                                       
...
C  BLOCK DATA SUBPROGRAM.                                                   0184
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     0191
     2 SRANGE                                                               0192
...
C     DATA RANGE/1.E35/, SRANGE/1.E35/, NIN/5/, NOUT/6/                     0206
...
C      REAL, INTEGER, AND LOGICAL CONTROL VARIABLES, IN THAT ORDER.         0217
     ...
     1 RSVMNX/2*1., 2*0./, RUSER/100*0./,                                   0219
     2 SRMIN/.01/                                                           0220
      DATA ICRIT/2*1/,                                                      0221
...
C      THE OUTPUT.  YOU MUST SPECIFY THE NAME IN THE FOLLOWING STATEMENT    0243
      DATA IAPACK/1HM, 1Hu, 1Hl, 1Ht, 1H1, 1Hq/!g2
C***********************************************************************    0245
C                                                                           0246
      END                                                                   0247
C++++++++++++++++ Multiq VERSION 2DP (March 1996) ++++++++++++++     
...
C-----------------------------------------------------------------------    0642
     ...
     1 NONNEG, ONLY1, PRWT, PRY, SIMULA, LUSER                              0646
      EXTERNAL BETAIN, DIFF, FFLAT, FISHNI, FORMF2, GAMLN, PGAUSS, 
     1         RANDOM, USEREX, USERLF, USERTR
      DIMENSION T(JT), G(1)                                                 0647
      DIMENSION IHOLER(6)                                                   0648
      COMMON /DBLOCK/ PRECIS, RANGE                                         0649
      COMMON /SBLOCK/ DFMIN, SRMIN,                                         0650
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     0651
     2 SRANGE                                                               0652
...
C-----------------------------------------------------------------------    0675
      ...
      userk=0.!g2
      ex=g(jg)*t(jt)!g2
      if (luser(29) .and. 2*jg.gt.ng) then!g2
         do 110 idset=1,iuser(20)!g2
            if (jt .le. iuser(20+idset)) go to 150!g2
  110    continue!g2
         call errmes (9, .true., iholer, nout)!g2
  150    ex=ex*ruser(50+idset)!g2
      end if!g2
  500 IF (EX .LT. EXMAX) USERK=formf2(jt,t,jg,g)*EXP(-EX)!ff
      RETURN                                                                0844
      END                                                                   0845
C++++++++++++++++ Splice Package VERSION 2DP (November 1991) ++++++++++++++     
...
C                                                                               
C                                                                               
C  Below is illustrated the case from the Splice Applications Package,          
C    which attempts to scale NDSET data sets together by adjusting the          
C    scale factor for the first (NDSET-1) data sets to get an optimal           
C    global fit to the data.  The scale factor for the last data set            
C    is left at 1.0.                                                            
C                                                                               
C  IUSER(20) = NDSET, the number of data sets.                                  
C            .LE. 0 means NDSET=1.                                              
C                                                                               
C       NLINF (see Sec. 4.1.2.4 of the Users Manual) can only have the          
C             following 3 values:                                               
C       NLINF = 2*NDSET-1 for 1 different constant baseline (or "dust           
C                         term") for each of the NDSET data sets.               
C                         This is probably the most common case.                
C                         (The last NDSET-1 parameters are always used          
C                          to rescale the first NDSET-1 data sets for           
C                          optimal splicing.)                                   
C             = NDSET for the same baseline parameter for                       
C                     all the data sets.  (Only reasonable if the               
C                     baseline term is the same and reproducible                
C                     over all data sets.)                                      
C             = NDSET-1 for no baseline parameter at all.                       
C                                                                               
C  IUSER(J), J=21,22,...,20+IUSER(20), = the subscript of the last data         
C                                        point of data set J in the Y           
...
C  ===================                                                          
C                                                                               
C  MYPAR is the dimension specification for the Y array in the                  
C        common block MAINLF below.  It must match the dimension                
C        specification for the Y array in the main subprogram.                  
C-----------------------------------------------------------------------    0867
...
C-----------------------------------------------------------------------    0869
     ...
     1 NONNEG, ONLY1, PRWT, PRY, SIMULA, LUSER                              0873
      EXTERNAL BETAIN, DIFF, FFLAT, FISHNI, FORMF2, GAMLN, PGAUSS, 
     1         RANDOM, USEREX, USERK, USERTR
      DIMENSION T(*), IHOLER(6)                                                 
C     ******************************************************************
      PARAMETER (MYPAR=5000)
C     ******************************************************************
      COMMON /MAINLF/ Y(MYPAR)                                                  
      COMMON /DBLOCK/ PRECIS, RANGE                                         0875
      COMMON /SBLOCK/ DFMIN, SRMIN,                                         0876
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     0877
     2 SRANGE                                                               0878
     ...
     2 LUSER(30)                                                            0887
      ...
      IF (JY.GT.MIN0(MYPAR,NY) .OR. JY.LE.0) CALL ERRMES (1,.TRUE.,             
     1                                                    IHOLER,NOUT)          
C-----------------------------------------------------------------------    0890
C  THE FOLLOWING STATEMENTS MUST BE REPLACED BY THE APPROPRIATE             0891
C      ONES FOR YOUR ADDITIVE FUNCTIONS.                                    0892
C-----------------------------------------------------------------------    0893
      IF (JLINF.LT.1 .OR. JLINF.GT.NLINF) CALL ERRMES (2,.TRUE.,                
     1                                                 IHOLER,NOUT)             
      NDSET=MAX0(1,IUSER(20))                                                   
      NBASE=NLINF-NDSET+1                                                       
      IF (NBASE.NE.0 .AND. NBASE.NE.1 .AND. NBASE.NE.NDSET) CALL ERRMES         
     1     (3,.TRUE.,IHOLER,NOUT)                                               
      IF (JLINF .LE. NBASE) THEN                                                
         JDSET=JLINF                                                            
         USERLF=1.                                                              
      ELSE                                                                      
         JDSET=JLINF-NBASE                                                      
      ...
      END IF                                                                    
C     ------------------------------------------------------------------        
C     If JY falls within data set JDSET, return.                                
C     ------------------------------------------------------------------        
      IF (JY.LE.IUSER(20+JDSET) .AND.                                           
     1    (JY.GT.IUSER(19+JDSET) .OR.JDSET.EQ.1)) RETURN                        
C     ------------------------------------------------------------------        
C     If there is only one baseline term for all data sets, and JLINF           
C     corresponds to this term, return.                                         
C     ------------------------------------------------------------------        
      IF (NBASE.EQ.1 .AND. JLINF.EQ.1) RETURN                                   
      USERLF=0.                                                                 
      RETURN                                                                0899
      END                                                                   0900
C++++++++++++++++ Splice Package VERSION 2DP (November 1991) ++++++++++++++     
...
C    is left at 1.0.                                                            
C                                                                               
C  In this example, USERNQ constrains the baseline terms (discussed             
C    below) to be nonnegative.                                                  
C                                                                               
C  IUSER(20) = NDSET, the number of data sets.                                  
C            .LE. 0 means NDSET=1.                                              
C                                                                               
...
C-----------------------------------------------------------------------    0920
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     0930
     2 SRANGE                                                               0931
...
C-------------------------------------------------------------------------------
      SUBROUTINE SETGRD (CQUAD,G,GMNMX,IGRID,IQUAD,MG,NG,NOUT,l29,i20)!g2
      logical l29!g2
      EXTERNAL BETAIN, DIFF, FFLAT, FISHNI, FORMF2, GAMLN, PGAUSS, 
     1         RANDOM, USEREX, USERK, USERLF, USERTR
...
C-------------------------------------------------------------------------------
      ...
      if (l29) then!g2
         ng1=ng/2!g2
         if (iquad.ne.1 .or. i20.lt.2 .or. 2*ng1.ne.ng) call !g2
     1        errmes (4,.true.,iholer,nout)!g2
         g(ng1+1)=g(1)!g2
      else!g2
         ng1=ng!g2
      end if!g2
      DELTA=(USERTR(GMNMX(2),1)-USERTR(GMNMX(1),1))/FLOAT(NG1-1)!g2
      DO 210 J=2,NG1!g2
        DUM=USERTR(G(J-1),1)+DELTA
        G(J)=USERTR(DUM,2)
        if (l29) g(j+ng1)=g(j)!g2
  210 CONTINUE
C-------------------------------------------------------------------------------
...
C-------------------------------------------------------------------------------
  300 IF (NG .LE. 2) GO TO 350
      ...
      DO 310 J=3,NG1!g2
        ...
        CALL ERRMES (2,.FALSE.,IHOLER,NOUT)
 5310   FORMAT (1X,1P10E13.3)
        WRITE (NOUT,5310) (G(K),K=1,NG)
        STOP
  315   DELOLD=DEL
  310 CONTINUE
...
C-------------------------------------------------------------------------------
  ...
  800 RETURN
      END
C++++++++++++++++ Multiq Package VERSION 2DP (March 1996) ++++++++++++++     
      function formf2 (jt, t, jg, g)
      parameter (mdset=30, mg1=44, mg2=2*(mg1-1))
      DOUBLE PRECISION PRECIS, RANGE
      LOGICAL DOCHOS, DOMOM, DOUSIN, DOUSNQ, LAST, NEWPG1,
     1 NONNEG, ONLY1, PRWT, PRY, SIMULA, LUSER
      EXTERNAL BETAIN, DIFF, FFLAT, FISHNI, GAMLN, PGAUSS, 
     1         RANDOM, USEREX, USERK, USERLF, USERTR
      save aformf
      dimension t(jt), g(*), aformf(mg2,mdset)
      DIMENSION IHOLER(6)
      COMMON /DBLOCK/ PRECIS, RANGE
      COMMON /SBLOCK/ DFMIN, SRMIN,
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),
     ...
     1 NEWPG1, NONNEG, ONLY1, PRWT, PRY, SIMULA,
     2 LUSER(30)
      DATA IHOLER/1HF, 1HO, 1HR, 1HM, 1HF, 1H2/
      if (.not.luser(4)) then
C        ------------------------------------------------------------------
C        This is the first call to formf2.  Fill array aformf(jg,jdset) with 
C          the squared form factors for grid point jg and data set jdset.
C        ------------------------------------------------------------------
         luser(4)=.true.
C        ------------------------------------------------------------------
C        You must modify the following for your squared form factors.  The 
C          example below simply sets them all to 1.0.
C        You have at your disposal the USER arrays, RUSER, etc.
C        For example, with a double grid (when LUSER(29)=T), 
C          RUSER(50+IDSET) = [q(IDSET)/q(1)]**2, IDSET=1,IUSER(20)
C          You can use RUSER(J), J=70,...,100 for your own input
C        ------------------------------------------------------------------
         if (luser(29)) then
C           ***************************************************************
C           This is the case of a double grid.
C           ***************************************************************
            do 110 jg=1,ng/2
               do 120 jdset=1,iuser(20)
C                 *********************************************************
C                 The following statement would have to evaluate the squared 
C                   form factor for for data set JDSET and grid point number 
C                   JG [you have its value, G(JG), at your disposal] on Grid 1 
C                   (the data-set-independent grid, e.g., the 
C                    angle-independent grid):
                  aformf(jg,jdset)=1.
C                 *********************************************************
  120          continue
  110       continue
            do 160 jdset=1,iuser(20)
               do 170 jg=ng/2+1,ng
C                 *********************************************************
C                 The following statement would have to evaluate the squared 
C                   form factor for for data set JDSET and grid point number 
C                   JG [you have its value, G(JG), at your disposal] on Grid 2 
C                   (the data-set-dependent grid, e.g., the diffusional grid):
C
                  aformf(jg,jdset)=1.
C
C                 *********************************************************
...
C
C           ***************************************************************
C           This is the normal case of a single grid (when LUSER(29)=F).
C           ***************************************************************
            do 210 jdset=1,iuser(20)
               do 220 jg=1,ng
C                 *********************************************************
C                 The following statement would have to evaluate the squared 
C                   form factor for data set JDSET and grid point number JG 
C                   [you have its value, G(JG), at your disposal]:
C
                  aformf(jg,jdset)=1.
C
C                 *********************************************************
...
C
C     **********************************************************************
C     This is the end of the changes that you have to make.  The following 
C       statements do not need to be changed.
C     **********************************************************************
      do 310 idset=1,iuser(20)
         if (jt .le. iuser(20+idset)) go to 350
  310 continue
      call errmes (1, .true., iholer, nout)
  350 formf2=aformf(jg,idset)
      end
...
C      IF NEQ=2 OR 3) WOULD HAVE TO HAVE BEEN PREVIOUSLY INPUT.             0270
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     0279
     2 SRANGE                                                               0280
...
C-----------------------------------------------------------------------    0407
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     0415
     2 SRANGE                                                               0416
...
C-----------------------------------------------------------------------    0557
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     0566
     2 SRANGE                                                               0567
...
C-----------------------------------------------------------------------    1000
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     1004
     2 SRANGE                                                               1005
...
C      SO THAT THE DATA IS ONLY READ ONCE.                                  1096
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     1105
     2 SRANGE                                                               1106
...
C-----------------------------------------------------------------------    1178
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     1188
     2 SRANGE                                                               1189
...
C-----------------------------------------------------------------------    1262
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     1270
     2 SRANGE                                                               1271
...
C-----------------------------------------------------------------------    1318
     ...
     1 NONNEG, ONLY1, PRWT, PRY, SIMULA, LUSER                              1322
      EXTERNAL BETAIN, DIFF, FFLAT, FISHNI, FORMF2, GAMLN, PGAUSS, 
     1         RANDOM, USEREX, USERK, USERLF
      COMMON /DBLOCK/ PRECIS, RANGE                                         1323
      COMMON /SBLOCK/ DFMIN, SRMIN,                                         1324
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     1325
     2 SRANGE                                                               1326
...
C      WHERE THE VARIANCE OF Y IS PROPORTIONAL TO (Y**2+1)/(4*Y**2).        1374
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     1382
     2 SRANGE                                                               1383
...
C-----------------------------------------------------------------------    1414
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     1436
     2 SRANGE                                                               1437
...
C-----------------------------------------------------------------------    1773
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     1796
     2 SRANGE                                                               1797
...
C-----------------------------------------------------------------------    1962
      ...
      LOGICAL SWAP                                                          1964
      EXTERNAL DIFF, FFLAT, FISHNI, FORMF2, GAMLN, PGAUSS, 
     1         RANDOM, USEREX, USERK, USERLF, USERTR
...
C      IS ONLY COUNTED IF ITS MAGNITUDE EXCEEDS SRMIN*(MAX. OF SOLUTN).     2426
     ...
     1 LBIND,MINEQ,LLSIGN)                                                  2428
      ...
      EXTERNAL BETAIN, DIFF, FISHNI, FORMF2, GAMLN, PGAUSS, 
     1         RANDOM, USEREX, USERK, USERLF, USERTR
...
C-----------------------------------------------------------------------    2469
      ...
      EXTERNAL BETAIN, DIFF, FFLAT, FORMF2, GAMLN, PGAUSS, 
     1         RANDOM, USEREX, USERK, USERLF, USERTR
...
C-----------------------------------------------------------------------    2810
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     2821
     2 SRANGE                                                               2822
...
C-----------------------------------------------------------------------    2869
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     2880
     2 SRANGE                                                               2881
...
C-----------------------------------------------------------------------    3105
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     3128
     2 SRANGE                                                               3129
...
C-----------------------------------------------------------------------    4270
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     4279
     2 SRANGE                                                               4280
...
C-----------------------------------------------------------------------    4734
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     4749
     2 SRANGE                                                               4750
...
C-----------------------------------------------------------------------    5069
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     5078
     2 SRANGE                                                               5079
...
C***********************************************************************    5116
  207 IF (IIN.LT.1 .OR. IIN.GT.100) GO TO 805                               5117
      RUSER(IIN)=RIN                                                        5118
      RETURN                                                                5119
...
C-----------------------------------------------------------------------    5698
     ...
     1 ALPST(2), EXMAX, GMNMX(2), PLEVEL(2,2), RSVMNX(2,2), RUSER(100),     5708
     2 SRANGE                                                               5709
