...
def cleanstr(i):
    toclean = ''.join(['\t', '\r', '\n', '\xb0', ','])
    return i.translate(str.maketrans('','', toclean))
...
def fix_columns(i_data, i_metadata):
    okay_clm_idcs = [i for i in range(0, np.shape(i_data)[1]) if not any(
        np.array(i_data)[:, i] == '******')]
...
def fix_rows(i_data):
    i_data = np.array([[n for n in row] for row in i_data if not any(
        np.array(row) == 'invalid point')])
...
def open_rhe(filename):
    ...
    datas = np.loadtxt(filename, encoding='latin1')
...
def check_columns(filename, columns):
    ...
    if not all(truth):
        print('# Some columns where not found: {}'.format(
            ' '.join([i for i in truthdict if truthdict[i] == False])))
        print('# Offending filename: {}'.format(filename))
        raise(ValueError(
            'One or more columns in the input are not found in the datafile(s).'))
    if all(truth):
        return True
    return None
...
def write_rheodicts(dsi_metadatas, i_metadatas, i_datas):
    for name in dsi_metadatas:
        ...
        metadatalist = ['{}:{}'.format(
            key, dsi_metadatas[name][key]) for key in dsi_metadatas[name]]
        metadatalist.append('\n')
        metadatalist.append('-----')
        for i in i_metadatas[name]:
            metadatalist.append("Interval {}".format(i))
            for key in i_metadatas[name][i]:
                metadatalist.append('{}:{}'.format(
                    key, i_metadatas[name][i][key]))
            metadatalist.append('\n')
            metadatalist.append('-----')
        ...
        for interval in i_datas[name]:
            ...
            filehead += '\t'.join(units)
            np.savetxt(cleanfname(name)+'/int'+format(int(interval),
                                                      '05d')+'.rhe', i_data, fmt="%s", header=filehead)
    ...
    for line in f:
        ...
        if in_imd == True:
            ...
            if splitcolon[0] in imd_constants:
                ...
                metadatas['{}'.format(
                    unannotated_metadata_counter)] = cleanstr(line)
