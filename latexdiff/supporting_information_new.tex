...
\clearpage
\section{Synthesis of pNIPAm-\emph{b}-ptBuAc-\emph{b}-pNIPAm precursors}
We prepared an ABA triblock precursor by successive polymerization of 
pNIPAm and ptBuAc, controlled by bis(methylpropionic acid) trithiocarbonate (BMAT), as shown in
...
Table \ref{tab:orgsec} for molecular weight averages $M_n$ and dispersities $\DJ$. 
In addition, chain extension of the BMAT-derived pNIPAm precursors with 
...
pNIPAm-\emph{b}-p\emph{t}BuAc-\emph{b}-pNIPAm and the (partially) deprotected products. 

\begin{table}
\centering
\caption{Characteristic SEC results for pNIPAm-\emph{b}-tBuAc-\emph{b}-pNIPAm.
Block lengths are based on theoretical molecular weight $M_{th}$ that is based on 
monomer conversion. $M_n$ and $\DJ$ are calculated from GPC traces after elution in THF.
N$_{n}$ designates a pNIPAm block of length $n$, T$_{m}$ a p\emph{t}BuAc block of length $m$.}

\begin{tabular}{lllll}
\toprule
&Polymer structure 	&$M_{th}$ (kDa) & $M_{n}$  (kDa)& $\DJ$\\
\midrule
&N$_\mathrm{26}$-N$_\mathrm{26}$									& 	6.2	& 	5.7	&	1.13 \\
&N$_\mathrm{25}$-N$_\mathrm{25}$									& 	6.1	& 	5.6	&	1.26 \\
&N$_\mathrm{24}$-N$_\mathrm{24}$									& 	5.6	& 	4.4	&	1.21 \\
...
\caption{RAFT synthesis of pNIPAm-\emph{b}-p\emph{t}BuAc-\emph{b}-pNIPAm, with p\emph{t}BuAc as precursor for the pAA block.
\label{syn:pNBN}}
\end{figure}
...
\end{subfigure}
\caption{(left) 400 MHz $^\mathrm{1}$H-NMR spectra of pNIPAm$_{\mathrm{52}}$ and 
pNIPAM$_{\mathrm{26}}$-\emph{b}-p\emph{t}BuAc$_{\mathrm{318}}$-\emph{b}-pNIPAM$_{\mathrm{26}}$ 
(right) SEC elugrams of pNIPAM and pNIPAM-\emph{b}-p\emph{t}BuAc-\emph{b}-pNIPAm in 
THF with refractive index detection. N$_{n}$ designates a pNIPAm block of length $n$, T$_{m}$ 
a p\emph{t}BuAc block of length $m$.}
...
\caption{FTIR spectra of (blue) N$_\mathrm{24}$T$_\mathrm{330}$N$_\mathrm{24}$, (orange)
...
resonance at 2935 cm$^{-1}$. N$_{n}$ designates a pNIPAm block of length $n$, T$_{m}$ 
a p\emph{t}BuAc block of length $m$.}
\label{fig:ir}
\end{figure}
...
N$_\mathrm{24}$-A$_\mathrm{220}$-SH (blue line). Polymers were eluted in aqueous 0.1M NaNO$_3$ on an Agilent 
PL AqueGel-OH 8$\mu$m column and detected by refractive index detection (left axis) and UV absorption (right axis). 
Numbers given are found $\mathrm{M}_\mathrm{n}$ values. }
\label{fig:aqs04}
\end{figure}
...
\end{subfigure}
\caption{Raw count rates in 90$^\circ$ scattering of a 292 mg mL$^{-1}$ solution of N$_\mathrm{24}$-A$_\mathrm{330}$-N$_\mathrm{24}$ (blue circles)
...
at zero salt. Error bars are the standard deviation of the count rate distribution. N$_{n}$ designates a pNIPAm block of 
length $n$, T$_{m}$ a p\emph{t}BuAc block of length $m$.}
\label{dls:CRs}
\end{figure}
